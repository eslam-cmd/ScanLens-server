...
let SubscriptionService = SubscriptionService_1 = class SubscriptionService {
    ...
    // ✅ الخطط
    async getAllPlans() {
        return Object.values(plans_config_1.PLANS);
    }
    ...
    async getUserPlan(userId) {
        ...
        const plan = plans_config_1.PLANS[user.plan];
        if (!plan)
            return plans_config_1.PLANS.free;
        return plan;
    }
    async getMySubscription(userId) {
        ...
        return {
            ...
            daysRemaining: isActive
                ? Math.ceil((user.subscriptionExpiresAt.getTime() - now.getTime()) /
                    (1000 * 60 * 60 * 24))
                : 0,
        };
    }
    ...
    async activateSubscription(userId, planId, billingCycle) {
        const plan = plans_config_1.PLANS[planId];
        if (!plan)
            throw new common_1.NotFoundException('Plan not found');
        const user = await this.prisma.user.findUnique({
        ...
        this.logger.log(`✅ Subscription activated for user ${userId} (${plan.name}) by Admin (${billingCycle})`);
        return {
            success: true,
            user: updatedUser,
            payment,
    ...
    // ✅ تجديد اشتراك
    async renewSubscription(userId, planId, billingCycle) {
        const plan = plans_config_1.PLANS[planId];
        if (!plan)
            throw new common_1.NotFoundException('Plan not found');
        const user = await this.prisma.user.findUnique({
        ...
        this.logger.log(`✅ Subscription renewed for user ${userId} (${plan.name}) (${billingCycle})`);
        return {
            success: true,
            user: updatedUser,
            payment,
    ...
    async processExpiredSubscriptions() {
        ...
        for (const user of expiredUsers) {
            ...
            await this.mailService.sendSubscriptionExpiredNotification(user.email, user.name || 'User', planName, user.subscriptionExpiresAt);
            expired.push({
                userId: user.id,
                email: user.email,
                plan: user.plan,
    ...
    async sendExpirationReminders() {
        ...
        for (const user of expiringUsers) {
            if (!user.subscriptionExpiresAt)
                continue;
            const daysRemaining = Math.ceil((user.subscriptionExpiresAt.getTime() - now.getTime()) /
                (1000 * 60 * 60 * 24));
            if (daysRemaining === 7 || daysRemaining === 3 || daysRemaining === 1) {
                const plan = plans_config_1.PLANS[user.plan];
                const planName = plan?.name || user.plan;
                await this.mailService.sendSubscriptionExpiringWarning(user.email, user.name || 'User', planName, user.subscriptionExpiresAt, daysRemaining);
                reminders.push({
                    userId: user.id,
                    email: user.email,
                    daysRemaining,
                });
                this.logger.log(`📧 Sent expiration reminder to ${user.email} (${daysRemaining} days remaining)`);
            }
        }
        return {
        ...
        return users.map((user) => ({
            ...
            daysRemaining: user.subscriptionExpiresAt
                ? Math.ceil((user.subscriptionExpiresAt.getTime() - now.getTime()) /
                    (1000 * 60 * 60 * 24))
                : 0,
            planDetails: plans_config_1.PLANS[user.plan] || null,
        }));
    }
    ...
    async getExpiringSubscriptions(days = 7) {
        ...
        return expiringUsers.map((user) => ({
            ...user,
            daysRemaining: Math.ceil((user.subscriptionExpiresAt.getTime() - now.getTime()) /
                (1000 * 60 * 60 * 24)),
            planDetails: plans_config_1.PLANS[user.plan] || null,
        }));
    }
    // ✅ التحقق من الصلاحيات
    async checkUserCapability(userId, action) {
        const user = await this.prisma.user.findUnique({
            where: { id: userId },
        });
        if (!user)
        ...
        if (!capability.enabled) {
            throw new common_1.ForbiddenException(`Action "${action}" is not available in your plan`);
        }
        await this.prisma.usageLog.create({
            data: {
                userId,
    ...
    // ✅ سجل الاستخدام
    async getUserUsageLogs(userId, limit = 50) {
    ...
    async getUserStats(userId) {
        ...
        if (!user)
            throw new common_1.NotFoundException('User not found');
        const plan = plans_config_1.PLANS[user.plan];
        const totalScans = user.websites.reduce((acc, website) => acc + website.scans.length, 0);
        ...
        return {
            ...
            websites: user.websites.map((w) => ({
                ...
                lastScan: w.scans.length > 0 ? w.scans[w.scans.length - 1].createdAt : null,
            })),
        };
    }
    // ✅ License Keys
    ...
    async purchaseLicense(data) {
        const plan = plans_config_1.PLANS[data.planId];
        if (!plan)
            throw new common_1.NotFoundException('Plan not found');
        ...
        const price = data.billingCycle === 'monthly' ? plan.price : plan.priceYearly;
    ...
    async verifyLicense(licenseKey, email) {
        ...
        if (!license.isActive) {
            throw new common_1.BadRequestException('License key is not active');
        }
        if (license.expiresAt && license.expiresAt < new Date()) {
            throw new common_1.BadRequestException('License key has expired');
        }
        await this.prisma.user.update({
            where: { email },
    ...
    async handleExpiredLicense(licenseKey) {
        const license = await this.prisma.license.findUnique({
            where: { key: licenseKey },
        });
        if (!license)
...
], SubscriptionService.prototype, "processExpiredSubscriptions", null);
