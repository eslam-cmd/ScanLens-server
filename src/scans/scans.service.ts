...
export class ScansService {
  ...
  private async cleanupExpiredScansForUser(
    userId: string,
    ...
    const effectiveRetentionDays =
      retentionDays ?? this.getRetentionDays(user.plan, user.role);

    if (effectiveRetentionDays === Infinity) {
    ...
    if (websiteIds.length === 0) {
      return { deleted: 0, retentionDays: effectiveRetentionDays };
    }
    ...
    this.logger.log(
      `🧹 Auto-cleaned ${deleted.count} expired scans for user ${userId} (${effectiveRetentionDays} days retention)`,
    );
...
3. Clean Code Snippet (e.g. Express/NestJS/Next.js or Nginx headers)`,
  ...
  async getUserHistory(userId: string) {
    ...
    if (retentionDays !== Infinity) {
      await this.cleanupExpiredScansForUser(userId, retentionDays);
    }
